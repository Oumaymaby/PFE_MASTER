...
if (isset($_SESSION['user_name'])){
    ...
    if(isset($_POST['abht'])){
        ...
        $rem_gen = htmlspecialchars($_POST["rema_bet"]);
        $avis_abht = htmlspecialchars($_POST["avis_abht"]);
        $etabli_abht = htmlspecialchars($_POST["etabli_par"]);
        $valide_abht = htmlspecialchars($_POST["validee_par"]);
        $approuve_abht = htmlspecialchars($_POST["approuve_par"]);
        $date_abht=date("Y-m-d");
        $date_bet_date=date("Y-m-d");
        $id= htmlspecialchars($_POST["id_prj"]);
        $avis_ab = new ABHT($avis_abht,$date_abht,$date_bet_date,$etabli_abht,$valide_abht,$approuve_abht,$rem_gen,$id);
        $p= new ABHT_Service();
        // echo $avis_abht;
        // var_dump($avis_abht === "2");
        if($avis_abht === "2"){
            $p->update_close($id);
            if($p->add($avis_ab)){
