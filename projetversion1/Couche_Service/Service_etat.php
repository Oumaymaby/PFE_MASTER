...
require_once '../Couche_metier/Etat_Classe.php';
...
class Etat_Service{
	...
	function nbtrait(){
		$st =	$this->db->prepare("select count(etat.etatdossier) from prj_inv.projets_investissement inv , prj_inv.ls_etat_dossier etat where inv.etatdossier = etat.id and etat.etatdossier='traitement en cours'");
	 	if ($st->execute()) {
	 	 		return $st->fetchAll();
	 		}
	 	 	else{
	...
	function nbnew(){
		...
		from prj_inv.projets_investissement inv , prj_inv.ls_etat_dossier etat 
	...
	function nbclose(){
		$st =	$this->db->prepare("select count(etat.etatdossier) from prj_inv.projets_investissement inv , prj_inv.ls_etat_dossier etat where inv.etatdossier = etat.id and etat.etatdossier='Dossier cloturer'");
	 	if ($st->execute()) {
	 	 		return $st->fetchAll();
	 		}
	 	 	else{
	...
	function projencours(){
		...
		$st =	$this->db->prepare("select inv.gid,inv.numero_dossier,inv.numero_archive,inv.commune,inv.province,inv.maitre_ouvrage,inv.intitule_projet,etat.etatdossier from prj_inv.projets_investissement inv , prj_inv.ls_etat_dossier etat where inv.etatdossier = etat.id and etat.etatdossier='Traitement en cours'");
	 	 	if ($st->execute()) {
	 	 		return $st->fetchAll();
	 		}
	 	 	else{
	...
	function projnew(){
		...
		$st =	$this->db->prepare("select inv.gid,inv.numero_dossier,inv.numero_archive,inv.commune,inv.province,inv.maitre_ouvrage,inv.intitule_projet,etat.etatdossier from prj_inv.projets_investissement inv , prj_inv.ls_etat_dossier etat where inv.etatdossier = etat.id and etat.etatdossier='Nouveau dossier'");
	 	 	if ($st->execute()) {
	 	 		return $st->fetchAll();
	 		}
	 	 	else{
